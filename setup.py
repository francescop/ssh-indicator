...
try:
    ...
    print >> sys.stderr, 'To build ssh-indicator you need https://launchpad.net/python-distutils-extra'
    sys.exit(1)
assert DistUtilsExtra.auto.__version__ >= '2.18', 'needs DistUtilsExtra.auto >= 2.18'

def update_config(libdir, values = {}):

    filename = os.path.join(libdir, 'ssh_indicator_lib/ssh_indicatorconfig.py')
    oldvalues = {}
    try:
        fin = file(filename, 'r')
...
def move_desktop_file(root, target_data, prefix):
    ...
    old_desktop_path = os.path.normpath(root + target_data +
                                        '/share/applications')
    old_desktop_file = old_desktop_path + '/ssh-indicator.desktop'
    desktop_path = os.path.normpath(root + prefix + '/share/applications')
    desktop_file = desktop_path + '/ssh-indicator.desktop'
    ...
    if not os.path.exists(old_desktop_file):
        ...
        desktop_file = desktop_path + '/extras-ssh-indicator.desktop'
        try:
    ...
    try:
        ...
        for line in fin:
            if 'Exec=' in line:
                ...
                line = "Exec=%s" % (target_scripts + 'ssh-indicator')
                if len(cmd) > 1:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        target_pkgdata = target_data + 'share/ssh-indicator/'
        target_scripts = '/' + os.path.relpath(self.install_scripts, self.root) + '/'

        values = {'__ssh_indicator_data_directory__': "'%s'" % (target_pkgdata),
...
DistUtilsExtra.auto.setup(
    ...
    version='0.1',
    license='GPL-3',
    author='Francesco Palazzini',
    author_email='palazzini.francesco@gmail.com',
    description='list your ssh servers',
    long_description="""
The SSH indicator allows you to list your ssh servers directly from your panel. 
  """,
    url='https://github.com/francescop/ssh-indicator',
